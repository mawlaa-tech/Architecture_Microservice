...
namespace Mango.Services.ShoppingCartAPI.Repository
{
    public class CartRepository : ICartRepository
    {
        ...
        {
            Cart cart = _mappaer.Map<Cart>(cartDto);

            // check if produt exist in database , if not create it
            var podInDb = await _db.Products
            ...
            return _mappaer.Map<CartDto>(cart);
        ...
        public async Task<bool> RemoveFromCart(int cartDetailsId)
        {
            try
            {
                ...
                int totalCountFromCartItems = _db.CartDetails.Where(u => u.CartHeaderId == cartDetails.CartHeaderId).Count();

                _db.CartDetails.Remove(cartDetails);
                if (totalCountFromCartItems == 1)
                {
